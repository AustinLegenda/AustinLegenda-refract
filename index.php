...
// Bootstrap DB (use existing connection helper)
[$pdo, $_s1, $_c1, $_cl1, $_t1] = LoadData::conn_report('41112');
[$_,   $_s2, $_c2, $_cl2, $_t2] = LoadData::conn_report('41117');

// View-models
$report = new Report();
$vmCC   = $report->currentConditionsView($pdo);   // Current Conditions
$vmNow  = $report->whereToSurfNowView($pdo);      // Where To Surf Now
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    h2, h3 { margin: 1.25rem 0 0.5rem; }
    table { border-collapse: collapse; width: 100%; margin: 0.75rem 0 1.25rem; }
    th, td { border: 1px solid #7774; padding: 8px 10px; vertical-align: top; }
    th { text-align: left; background: #eee4; }
...
<section aria-labelledby="ns-heading">
  ...
  <table>
    ...
    <?php if (!empty($vmCC['rows'])): ?>
      ...
      <?php endforeach; ?>
    <?php else: ?>
      <tr><td colspan="5"><em>No current buoy data available.</em></td></tr>
    <?php endif; ?>
    </tbody>
  </table>
  ...
  <table>
    <thead>
      <tr><th>Name</th><th>Wave</th><th>Tide</th><th>Wind</th></tr>
    </thead>
    <tbody>
      <?php if (!empty($vmNow['best'])): ?>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <tr><td colspan="4"><em><?= htmlspecialchars($vmNow['message'] ?? 'Conditions are less than optimal at this time.') ?></em></td></tr>
      <?php endif; ?>
    </tbody>
  </table>
